...
//console.log(getAllTask)
    ...
    const pendingTask=getAllTask[0]?.allTask?.filter(task=>task.status=='pending')
    const onGoingTask=getAllTask[0]?.allTask?.filter(task=>task.status=='inProgress')
    const completedTask=getAllTask[0]?.allTask?.filter(task=>task.status=='completed')
...
module.exports={findAllTask,checkAdmin,getAllUser, getAllProject}